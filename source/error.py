...
def compare_cage_lists(cids_m, cids_c):
    failed = False
    set_cids_m, set_cids_c = set(cids_m), set(cids_c)
    if len(set_cids_c) < len(cids_c):   #Check for duplicate CIDS
        for cid in cids_c:
            if cids_c.count(cid) > 1:
                err_dup_cid(cid)
                failed = True
    #Highest priority error: cage in mouse sheet that does not exist in cage sheet
    diff = set_cids_m.difference(set_cids_c)

    #2: empty cage(exists in cage sheet but not mouse sheet)
